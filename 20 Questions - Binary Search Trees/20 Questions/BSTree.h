...
public:
    ...
    void insertString(value s1, value s2, int x1); //declare a fucntion to insert a specified string into a specific place
    //string 1: (string to be inserted) / string 2: (string to be parent) / int: 0 for left child, 1 for right child.
    void choose();
};
...
template <class value>
void BSTree<value>::insertString(value s1, value s2, int x1)
{
    BST_node* n1 = new BST_node; //create a pointer to a new node
    
    
    BST_node* parent; //create a pointer to a "parent" node
    parent = NULL; //initialize the parent to NULL
    
    n1->data = s1; //enter the inputed 'data' into the middle of the node 'n'
    n1->left = NULL; //'n' will be a leaf, so it will have no left child
    n1->right = NULL; //'n' will be a leaf, so it will have no right child
    ...
    if ( root == NULL ){ //if the tree is empty, we just set the new node to the root
        ...
        //find location fo parent in the tree
        
        
        //make n1 the right child of the parent if x1 = 1, and make it the left child if x1 = 0.
